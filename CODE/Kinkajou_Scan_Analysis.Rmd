...
residence_by_region_relative_density_stan_fit <- 
  ulam(
    alist(
      Minutes_Per_Hour ~ dzagamma2(p, mu, scale),
      logit(p) <-  ap + ap_region[Region] + bp_Average_Density * Average_Density + bp_Relative_Density * Relative_Density,	
      log(mu)  <-  am + am_region[Region] + bm_Average_Density * Average_Density + bm_Relative_Density * Relative_Density,
      #priors below
      ...
      c(ap_Region,am_Region)[Region] ~ dnorm(0, 2),
      c(bp_Average_Density,bm_Average_Density) ~ dnorm(0, 2),
      c(bp_Relative_Density,bp_Relative_Density) ~ dnorm(0, 2),	
      scale ~ dcauchy(0,2)
    ),
    data = na.omit(combined_data_region), cores=4 , chains=4 , warmup=1500, iter=3000, 
...
summary(residence_by_region_relative_density_stan_fit)
```
# Brendan recomends using a 0 conditional (?) gamma distribution. Look at papers with Joan Silk for an example
...
post_samples %>% 
  pivot_longer(Region1:lp__, names_to = "Coefficient_Type", values_to = "Estimated_Effect") %>%
  filter(Coefficient_Type %in% c("Region1", "Region2", "Region3", "b_Total_Flowers", "b_Relative_sDensity")) %>%
  ggplot() + 
  stat_density_ridges(aes(x = Estimated_Effect, y = Coefficient_Type), scale = 1, quantile_lines = TRUE, quantiles = c(0.055, 0.945)) + #the quantile lines on total flowers look wrong . . .
  geom_vline(xintercept = 0, color = "red", linetype = "dashed") +
  theme_classic() +
