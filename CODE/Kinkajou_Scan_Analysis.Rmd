...
transition_array <- transition_arrays[["28"]][,,203]
flower_data <- filter(combined_data_region, Study_night == 28)
...
get_transition_info <- function(transition_array, flower_data){
  ...
  decisions <- mutate(decisions, adj_flowers_choice = arrange(flower_data, Region)$Adjusted_Count[To], #get the adjusted flower count of the region moved to for each decision.
                      region_avoid = mapply(FUN = function(to, from){ which(! c(1,2,3) %in% c(to, from))}, to = To, from = From), #get the region avoided by checking the regions transitioned from and the the region transitioned to against the possible regions (1,2,3; not generalizable to more regions)
                      times = mapply(function(data, dim1,dim2,dim3) data[dim1,dim2,dim3], dim1 = From, dim2=To, dim3=Minute, MoreArgs = list(data = decision_transitions)
  ))
}
```
...
```
Bt plotting the posterior distribution of parameter coeficcients, 
