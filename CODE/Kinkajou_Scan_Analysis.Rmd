...
simulated_scans <- all_kinkajou_scans %>% select(timestamp, Region1_total, Region2_total, Region3_total, Study_night)

for(night in names(transition_arrays)){
  simulated_scans[simulated_scans$Study_night == night,2:4] <- t(transition_arrays[[night]][4,-4,]) #indexes of transition arrays gets the number of entries into each region (vector of length 3) for each minute of a given night
}
