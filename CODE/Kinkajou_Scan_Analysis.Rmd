...
get_decision_info <- function(transition_array, flower_data){
  ...
  decisions <- mutate(decisions, relative_density_to = arrange(flower_data, Region)$Relative_Density[To], #get the relative flower density of the region moved to for each decision.
...
departures_long <- data.frame(matrix(nrow = 0, ncol = 6))
...
decisions <- mutate(decisions, Success = relative_density_diff > 0, From_To = factor(mapply(paste, From, To, MoreArgs = list(sep = "_"))))
```
...
decision_model <- ulam(
  alist(
    ...
    save> logit(p) <-  ap + ap_From_To[From_To] + bp_flowers * relative_density_diff,
    #priors below
...
ggplot(decision_predictions, aes(x = Predicted_Probability, y = reorder(factor(Observation_Number), Relative_Density_Diff), color = factor(Success))) + 
  ...
  labs(x = "Predicted Probability", y = "Relative Density Difference") +
  coord_flip() + 
  theme_classic() + 
...
social_model <- ulam(
  alist(
    ...
    c(bp_n, bp_Neightbors) ~ dnorm(0, 2), #prior distributions on effect of number of kinkajous in the region of interest and number of kinkajous in other regions on departure rates
